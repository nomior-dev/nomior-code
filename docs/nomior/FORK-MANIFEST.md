...
| `apps/web/src/components/CommandPalette.tsx`                  | one action per `NOMIOR_PAGES` entry plus an Instances action, so every surface reachable from the sidebar is reachable from the palette                                                                                    |
| `apps/web/src/components/settings/settingsSearch.ts`          | `/settings/instances` added to `SettingsPath` and the section labels — Instances is configuration, so it lives with the other configuration                                                                                |
| `apps/web/src/components/settings/SettingsSidebarNav.tsx`     | the `/settings/instances` icon entry, keeping the icon record total over `SettingsPath`                                                                                                                                    |
