...
| `apps/server/src/persistence/Migrations.ts`                   | migration registration: import + one `migrationEntries` entry per Nomior migration (044 context broker, connectors, scheduler, review)                       |
| `apps/server/package.json`                                    | script registration (`nomior:eval-retrieval`) and `googleapis` + `google-auth-library` deps for the Google connectors (loaded through a dynamic-import seam) |
...
| `apps/web/src/components/sidebar/SidebarChrome.tsx`           | sidebar entry + footer-page detection for the `/nomior` panels                                                                                               |
...
- the sidebar logo mark (`SidebarChrome.tsx` `aria-label="T3"` describes the
  still-upstream logo asset; both change together in the asset rebrand)
