...
Local desktop development uses `nomior-dev://app`, while packaged builds use `nomior://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `nomior-dev://app`; the production Clerk instance should only need
`nomior://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.nomior.nomiorcode`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.nomior.nomiorcode` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.nomior.nomiorcode`.
...
T3CODE_PORT=13773 \
  "/Applications/Nomior Code (Alpha).app/Contents/MacOS/Nomior Code (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Nomior Code (Alpha).app"
codesign -d --entitlements :- "/Applications/Nomior Code (Alpha).app"
```
