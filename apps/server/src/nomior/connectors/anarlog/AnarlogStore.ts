 ...
 * version first, and refuse to interpret tables outside the tested
 * version range (see `AnarlogSchema.ts`).
 *
...
export const openAnarlogStore = Effect.fnUntraced(function* (storePath: string) {
  const db = yield* Effect.acquireRelease(
    Effect.try({
      try: () => new NodeSqlite.DatabaseSync(storePath, { readOnly: true }),
  ...
  const listSessionBundlesSince: AnarlogStoreReader["listSessionBundlesSince"] = (since, limit) =>
    Effect.gen(function* () {
      ...
      const sessionRows = yield* query("listSessions", sessionsSinceSql, [
        sinceUpdatedAt,
        sinceUpdatedAt,
        sinceSessionId,
        limit,
