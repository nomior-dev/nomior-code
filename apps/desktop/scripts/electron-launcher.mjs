// This file mostly exists because we want dev mode to say "Nomior Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Nomior Code (Dev)" : "Nomior Code (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.nomior.nomiorcode.dev.${devBundleIdSuffix || "local"}`
  : "com.nomior.nomiorcode";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["nomior-dev"] : ["nomior"];
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Nomior Code name.
