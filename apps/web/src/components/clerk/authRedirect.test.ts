  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "nomior://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "nomior://app/#/settings/connections",
      signUpForceRedirectUrl: "nomior://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("nomior-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "nomior-dev://app/#/settings/general",
      signUpForceRedirectUrl: "nomior-dev://app/#/settings/general",
    });
  });
});
