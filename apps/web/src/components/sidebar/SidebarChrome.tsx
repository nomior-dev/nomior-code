...
import { splitBrandLockup } from "../../branding.logic";
import { useEnvironmentIdentificationMode } from "../../hooks/useSettings";
import { NomiorCodeMark } from "../../nomior/BrandMark";
...
function SidebarBrand({ onBackdrop }: { onBackdrop: boolean }) {
  const lockup = splitBrandLockup(APP_BASE_NAME);
  return (
    <Link
      ...
      to="/"
    >
      <NomiorCodeMark className="shrink-0" size={18} />
      <span className="truncate text-sm font-semibold tracking-tight">{lockup.name}</span>
      {lockup.product ? (
        <span
          className={cn(
